  ...
  >
    <template #header>
      <div class="captcha-head">
        <p class="captcha-kicker">Human Check</p>
    ...
    <div class="captcha-box" v-loading="loading">
      ...
      <div class="captcha-actions">
        <span class="captcha-tip">{{ errorText || "验证通过后将继续登录或注册" }}</span>
...
const loadCaptcha = async () => {
  ...
  resetDrag()
  try {
    const { data } = await appClient.post("/app/auth/captcha/get", {
      captchaType: "blockPuzzle",
      clientUid: `app-${Date.now()}-${Math.random().toString(16).slice(2)}`
    })
...
const checkCaptcha = async () => {
  ...
  try {
    ...
    const { data } = await appClient.post("/app/auth/captcha/check", {
      captchaType: "blockPuzzle",
      token: captcha.token,
      pointJson
    })
...
.captcha-progress {
  ...
  z-index: 0;
  border-radius: 999px;
  background: linear-gradient(90deg, rgba(109, 103, 255, 0.22), rgba(142, 232, 200, 0.55));
}
...
.captcha-slider {
  ...
  cursor: grab;
  touch-action: none;
  box-shadow: 0 4px 0 rgba(52, 45, 105, 0.14);
}
