...
const refreshAccessToken = async () => {
  ...
  const { data } = await refreshClient.post('/app/auth/refresh-token', {
...
const handleBusinessCode = async (response) => {
  ...
  if (payload.code === 602) {
    if (!originalConfig._retry && !isRefreshRequest(originalConfig) && !isLoginRequest(originalConfig) && store.refreshToken) {
      ...
      try {
        ...
        originalConfig.headers['access-token'] = nextAccessToken
        return appClient.request(originalConfig)
      } catch (_) {
...
appClient.interceptors.request.use((config) => {
  ...
  if (store.accessToken) {
    ...
    config.headers['access-token'] = store.accessToken
  }
  return config
})

appClient.interceptors.response.use(
  async (response) => handleBusinessCode(response),
  (error) => {
...
export { appClient, isHandledBusinessError }
